...
namespace SushiApp.PaginaAdmin
{
    public partial class mantenedor_categoria1 : System.Web.UI.Page
    {
        ...
        {     
            try
            {
                var listadtoAdministrador = categoriaCliente.obtenerCategoriaProducto();
                var nuevolistadtoAdministrador = (from o in listadtoAdministrador
                                                      //orderby o.clienteId
                                                  select new
                                                  {
                                                      Id = o.categoriaProductoId,
                                                      Nombre = o.nombreCategoria,                                                      
                                                  }).ToList();

                gv1.DataSource = nuevolistadtoAdministrador;
                gv1.DataBind();
            }
            catch (Exception)
            {

                Response.Write("<script>alert('No se pudo cargar GridView Categoría-Producto');</script>");
            }
